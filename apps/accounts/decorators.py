    ...
    def decorator(view_func):
        ...
        def _wrapped(request, *args, **kwargs):
            if not request.user.is_authenticated:
                # En los módulos con cookie aislada debemos entrar por su login
                # propio. Enviar siempre al login genérico puede crear un ciclo:
                # /gerente/ usa sid_gerente, mientras /accounts/ ve sessionid y
                # vuelve a redirigir inmediatamente a /gerente/.
                if request.path.startswith("/gerente/"):
                    return redirect("gerente:login_gerente")
                if request.path.startswith("/mesero/"):
                    return redirect("mesero:login_mesero")
                if request.path.startswith("/cocina/"):
