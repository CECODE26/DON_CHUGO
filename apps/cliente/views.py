...
@require_POST
def entrar_como_invitado(request, mesa_id):
    """Crea o recupera la sesión identificada con el nombre del cliente.

    El QR identifica la mesa y el nombre permite localizar su consumo en Caja.
    No requiere cuenta, contraseña ni PIN. Si el dispositivo ya tiene una sesión
    activa, se reutiliza y se actualiza el nombre sin duplicarla.
    """
    from django.db import transaction

    mesa = get_object_or_404(Mesa, pk=mesa_id)
    alias = " ".join(request.POST.get("alias", "").strip().split())[:50]
    if not alias:
        return render(request, "cliente/bienvenida.html", {
            "mesa": mesa,
    ...
    token_actual = request.COOKIES.get("mm_session")
    if token_actual:
        sesion_existente = SesionCliente.objects.filter(
            token_cookie=token_actual,
            mesa=mesa,
            estado="activa",
        ).first()
        if sesion_existente:
            nombre_ocupado = SesionCliente.objects.filter(
                mesa=mesa, alias__iexact=alias, estado="activa"
            ).exclude(pk=sesion_existente.pk).exists()
            if nombre_ocupado:
                return render(request, "cliente/bienvenida.html", {
                    "mesa": mesa, "alias_previo": alias,
                    "error": "Ese nombre ya está siendo usado en esta mesa.",
                }, status=409)
            if sesion_existente.alias != alias:
                sesion_existente.alias = alias
                sesion_existente.save(update_fields=["alias"])
    ...
    with transaction.atomic():
        ...
        modalidad, _ = ModalidadIngreso.objects.get_or_create(descripcion="qr")
...
def agregar_carrito(request):
    ...
    if unidades is not None:
        if not isinstance(unidades, list) or len(unidades) != cantidad or not all(isinstance(x, list) for x in unidades):
            return JsonResponse({"ok": False, "error": "La selección por unidad no es válida"}, status=400)
        selecciones_crudas = unidades
    else:
        if not isinstance(modificadores_ids, list):
            return JsonResponse({"ok": False, "error": "Modificadores inválidos"}, status=400)
        selecciones_crudas = [modificadores_ids]
    notas = data.get("notas", "")
    producto = get_object_or_404(Producto, pk=producto_id, disponible=True)
    from apps.menu.models import OpcionModificador

    # Validación en servidor: cada opción debe estar activa y pertenecer a un
    # grupo asignado al producto. También se respetan grupos obligatorios y el
    # máximo de selecciones; no dependemos únicamente del JavaScript del modal.
    grupos = list(producto.grupos_modificadores.prefetch_related("opciones").all())
    todos_ids = []
    try:
        selecciones_ids = []
        for seleccion in selecciones_crudas:
            ids = list(dict.fromkeys(int(pk) for pk in seleccion))
            selecciones_ids.append(ids)
            todos_ids.extend(ids)
    except (TypeError, ValueError):
        return JsonResponse({"ok": False, "error": "Modificadores inválidos"}, status=400)

    opciones_validas = list(OpcionModificador.objects.filter(
        pk__in=set(todos_ids),
        activo=True,
        grupo__productos=producto,
    ).select_related("grupo").distinct())
    opciones_por_id = {op.pk: op for op in opciones_validas}
    if any(pk not in opciones_por_id for pk in todos_ids):
        return JsonResponse({"ok": False, "error": "Una de las opciones seleccionadas no es válida"}, status=400)

    selecciones_validadas = []
    for indice, ids in enumerate(selecciones_ids, start=1):
        opciones = [opciones_por_id[pk] for pk in ids]
        seleccionadas_por_grupo = {}
        for opcion in opciones:
            seleccionadas_por_grupo.setdefault(opcion.grupo_id, []).append(opcion)
        for grupo in grupos:
            seleccionadas = seleccionadas_por_grupo.get(grupo.pk, [])
            sufijo = f" para la unidad {indice}" if unidades is not None and cantidad > 1 else ""
            if grupo.es_obligatorio and not seleccionadas:
                return JsonResponse({
                    "ok": False,
                    "error": f'Selecciona una opción en "{grupo.nombre_grupo}"{sufijo}',
                }, status=400)
            limite = 1 if grupo.tipo == "única" else grupo.max_selecciones
            if limite and len(seleccionadas) > limite:
                return JsonResponse({
                    "ok": False,
                    "error": f'Solo puedes elegir {limite} opción(es) en "{grupo.nombre_grupo}"{sufijo}',
                }, status=400)
        selecciones_validadas.append(opciones)

    items = _get_carrito(request)
    for opciones in selecciones_validadas:
        opciones_detalle = [
            {"id": op.pk, "nombre": op.nombre_opcion, "extra": float(op.precio_extra)}
            for op in opciones
        ]
        precio_extra = sum(op["extra"] for op in opciones_detalle)
        cantidad_linea = 1 if unidades is not None else cantidad
        items.append({
            "producto_id": producto.pk, "nombre": producto.nombre,
            "precio_unitario": float(producto.precio), "cantidad": cantidad_linea,
            "modificadores": opciones_detalle, "notas": notas,
            "subtotal": (float(producto.precio) + precio_extra) * cantidad_linea,
        })
    ...
    with transaction.atomic():
        ...
        if idempotency_key:
            ...
            if ya_existe is None:
                confirmacion = PedidoConfirmacion.objects.select_related("pedido").filter(
                    token=idempotency_key
                ).first()
                ya_existe = confirmacion.pedido if confirmacion else None
        ...
        # Un solo pedido acumulado por invitado: si ya existe, las nuevas líneas
        # se agregan al mismo registro. Al recibir una adición vuelve a
        # "recibido" para que Caja la procese y la alerta sea visible.
        pedido = (
            Pedido.objects.select_for_update()
            .filter(sesion=sesion_locked)
            .exclude(estado="cancelado")
            .order_by("-fecha_hora_ingreso")
            .first()
        )
        es_adicion = pedido is not None
        if pedido is None:
            pedido = Pedido.objects.create(
                sesion=sesion_locked,
                modalidad=sesion_locked.modalidad_ingreso,
                token_idempotencia=idempotency_key or None,
            )
        else:
            pedido.estado = "recibido"
            pedido.cantidad_adiciones += 1
            pedido.fecha_hora_entrega = None
            pedido.empleado_entrega = None
            pedido.save(update_fields=[
                "estado", "cantidad_adiciones", "fecha_hora_entrega",
                "empleado_entrega", "fecha_hora_actualizacion",
            ])

        if idempotency_key:
            PedidoConfirmacion.objects.get_or_create(
                token=idempotency_key,
                defaults={"pedido": pedido},
            )
        for item in items:
            ...
            for op_data in item.get("modificadores", []):
                try:
                    ...
                    pass
        _save_carrito(request, [])
    return JsonResponse({"ok": True, "pedido_id": pedido.pk, "adicion": es_adicion})
...
def desglose_pago(request):
    """
    GET JSON — devuelve el desglose del cobro para el modal de pedidos.

    Parámetros query:
        tipo        -- "individual" o "grupal".
        propina_pct -- porcentaje de propina a calcular sobre el subtotal bruto.

    Retorna:
        {ok, items, subtotal_bruto, descuentos, propina, total}
    """
    from apps.pedidos.models import Pedido as _Pedido

    sesion = request.sesion_cliente
    tipo = (request.GET.get("tipo") or "individual").strip().lower()
    try:
        propina_pct = Decimal(str(request.GET.get("propina_pct", "0") or "0"))
        if propina_pct < 0:
    ...
    if tipo == "grupal":
        ...
        pedidos_qs = sesion.pedidos.all()

    pedidos = list(
        pedidos_qs
        .prefetch_related("detalles__producto", "detalles__modificadores", "detalles__promocion")
        .exclude(estado="cancelado")
        .order_by("-fecha_hora_ingreso")
    )
    ...
    for pedido in pedidos:
        for detalle in pedido.detalles.all():
            extras = sum(
                ...
                for m in detalle.modificadores.all()
            )
            precio_lista = (detalle.producto.precio * detalle.cantidad) + extras
            subtotal_bruto += precio_lista
            subtotal_neto += detalle.subtotal_calculado
            descuento = precio_lista - detalle.subtotal_calculado
            if descuento > 0:
            ...
            items.append({
                ...
                "subtotal": float(detalle.subtotal_calculado),
                "promo_aplicada": detalle.promocion.titulo if detalle.promocion else None,
            })

    if descuento_total < 0:
        descuento_total = Decimal("0.00")

    propina = (subtotal_bruto * (propina_pct / Decimal("100")))
    propina = propina.quantize(Decimal("0.01"))
    total = subtotal_bruto - descuento_total + propina
    ...
    return JsonResponse({
        ...
        "descuentos": float(descuento_total),
...
def estado_sesion(request):
    ...
    sesion = request.sesion_cliente
    # Tras el cobro la sesión se cierra automáticamente. Conservamos ``pagada``
    # como estado compatible con registros antiguos todavía no normalizados.
    pagada = sesion.estado in ("pagada", "cerrada")
