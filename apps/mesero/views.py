...
def _post_pago_mesa(mesa, user=None):
    """
    Finaliza automáticamente las sesiones que acaban de pagarse.

    Las sesiones pasan primero por ``pagada`` para que ``SesionCliente.save()``
    marque sus pedidos como pagados. Este helper las mueve después a ``cerrada``.
    Si no quedan otras sesiones activas, también libera la mesa y limpia el PIN.
    En cobros individuales conserva la mesa ocupada mientras existan clientes con
    cuentas abiertas y registra cuánto queda pendiente.

    Debe invocarse dentro de ``transaction.atomic()`` después de registrar el pago.
    """
    from apps.mesas.models import AlertaMesero

    # Serializar la actualización final de la mesa contra cobros simultáneos de
    # dos sesiones distintas. También cerramos sesiones pagadas antiguas que
    ...
    if not sesiones_activas:
        ...
        mensaje = (
            f"Mesa {mesa.numero_mesa} pagada y liberada automáticamente."
        )
    else:
        # Un cobro individual no debe expulsar a los demás clientes de la mesa.
        ...
        mesa.save(update_fields=["estado", "nota_cierre"])
...
def procesar_pago(request):
    ...
    # Nunca registrar como cobrada una cuenta vacía. Además de ser un dato
    # contable inválido, antes permitía cerrar sesiones sin ningún consumo.
    if total <= Decimal("0.00"):
        return _volver_pago(
            "No se puede cobrar una cuenta de $0.00. Agrega productos al pedido antes de facturar."
        )
    ...
    if "MIXTO" in desc:
        ...
        if not monto_str:
            return _volver_pago("Ingresa el monto recibido en efectivo.")
        try:
        ...
        if monto_recibido <= Decimal("0.00"):
            return _volver_pago("El monto recibido debe ser mayor que $0.00.")
    ...
    with transaction.atomic():
        if sesion_id:
            ...
            # Finalizar la sesión pagada y liberar la mesa si era la última.
    ...
    if getattr(request, "pago_redirect_admin", False):
        return redirect("admin:pedidos_solicitudpago_comprobante", sol.pk)
...
def cancelar_pedido(request):
    ...
    if pedido.estado in ("entregado", "pagado", "cancelado"):
...
def _ticket_context(sol, user):
    ...
    restaurante = {
        "nombre":    cfg("restaurante_nombre", "Don Chugo"),
