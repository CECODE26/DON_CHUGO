...
### ❌ Pantalla en blanco en VS Code Simple Browser (localhost)

Si la app responde por `curl` pero en el navegador integrado de VS Code aparece en blanco, normalmente es por la cabecera anti-iframe `X-Frame-Options`.

Para desarrollo local, este proyecto deja `XFrameOptionsMiddleware` activo **solo en producción** (`DEBUG=False`).

Verifica:

1. Que el `.env` local tenga `DEBUG=True`
2. Reconstruir el contenedor web después de cambios en settings:
...
```bash
curl -I http://localhost:8002/bienvenida/
```

No debe aparecer `X-Frame-Options: DENY` en desarrollo.
